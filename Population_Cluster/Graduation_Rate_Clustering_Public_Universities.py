...
# Load the graduation rate dataset
file_path_grad = 'CSV_Graduation_Rate_All_Universities.csv'
grad_data = pd.read_csv(file_path_grad)

# Load the population diversity dataset with clusters
file_path_div = 'Updated_Population_Diversity_with_Clusters.csv'
div_data = pd.read_csv(file_path_div)

# Merge the two datasets on 'institution name'
merged_data = pd.merge(grad_data, div_data, on='institution name', how='inner')
print(merged_data.info())
# Filter public schools
public_school_data = merged_data[merged_data['HD2022.Sector of institution'] == 'Public, 4-year or above']
print(public_school_data.info())
public_school_data = public_school_data.dropna(subset=['DRVGR2022.Graduation rate, total cohort'])
print(public_school_data.info())
...
df_selected.to_csv('public_schools_new_clusters.csv', index=False)

# Load the dataset
file_path = 'public_schools_new_clusters.csv'
data = pd.read_csv(file_path)

institution_names = data['institution name']

# Drop the 'institution name' column for clustering
...
df_selected = df_selected.loc[df_encoded.index]
...
plt.grid(True)
plt.savefig('public_university_clusters_combined_kmeans_2d_no_scaling.png')
plt.show()

df_selected['institution name'] = institution_names
# Save the updated DataFrame to a new CSV file
df_selected.to_csv('public_schools_new_clusters_no_scaling.csv', index=False)